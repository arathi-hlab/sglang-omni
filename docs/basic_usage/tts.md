...
| `initial_codec_chunk_frames` | int | `null` | Optional first codec chunk size for streaming TTFA / playback-continuity tuning. When omitted, each model applies its own default: Qwen3-TTS Base and CustomVoice use `8`, Higgs TTS uses `20`, MOSS-TTS Local uses `5`, and ZONOS2 uses `40`. An explicit `0` uses the model's steady chunk size from the start. Ming-Omni-TTS rejects the field entirely |
| `non_streaming_mode` | bool | `null` | Qwen3-TTS CustomVoice compatibility flag. When `true`, keeps the packed-text prompt and whole-utterance vocoder path. Omitted CustomVoice requests use the same incremental `stream_to=vocoder` path as Base. VoiceDesign stays non-streaming |
